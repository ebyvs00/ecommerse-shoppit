...
class Cart(models.Model):
    ...
    def save(self, *args, **kwargs):
        # ✅ Enforce one unpaid cart per user at Python level
        if self.user and not self.paid:
            existing = Cart.objects.filter(user=self.user, paid=False).exclude(pk=self.pk)
            if existing.exists():
                raise ValueError("User already has an unpaid cart.")
        super().save(*args, **kwargs)
